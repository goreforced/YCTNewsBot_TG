...
post_count = 0
error_count = 0
duplicate_count = 0 # Новый счётчик дублей
last_post_time = None
...
def get_article_content(url):
 ...
 logger.info(f"Запрос к OpenRouter для {url}")
 try:
 ...
 logger.info(f"Получен ответ от OpenRouter: {content[:50]}...")
 if "\n" in content:
...
Пропущено дублей: {duplicate_count} # Добавлено
Ошибок: {error_count}
...
def webhook():
 ...
 file_name = update['message']['document']['fileperienza.org/file_name = update['message']['document']['file_name']
